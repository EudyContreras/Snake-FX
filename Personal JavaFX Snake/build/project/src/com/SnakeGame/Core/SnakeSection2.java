...
public class SnakeSection2 extends SectionMain{
	double particleLife;
	double particleSize;
	...
	public SnakeSection2(Player2 snake, SnakeGame game, Pane layer, Circle node, double x, double y, GameObjectID id, PlayerMovement Direction, int numericID) {
		...
		if (this.numericID <= 0) {
			if (Direction == PlayerMovement.MOVE_UP) {
				...
				tail = new SnakeTail2(this,game, game.getBottomLayer(), new Circle(Settings.SECTION_SIZE-5,new ImagePattern(GameImageBank.snakeTail)), this.x, this.y, GameObjectID.SnakeTail, PlayerMovement.MOVE_UP);
				game.getObjectManager().addObject(tail);
				snake.setTail(tail);
				snake.setNeighbor(this);
				if(this.numericID == Player2.NUMERIC_ID){
				...
				this.velY = snake.getVelY();
				setLastPosition(new Point2D(this.x,this.y));
				tail = new SnakeTail2(this,game, game.getBottomLayer(), new Circle(Settings.SECTION_SIZE-5,new ImagePattern(GameImageBank.snakeTail)), this.x, this.y, GameObjectID.SnakeTail, PlayerMovement.MOVE_DOWN);
				game.getObjectManager().addObject(tail);
				snake.setTail(tail);
				snake.setNeighbor(this);
				if(this.numericID == Player2.NUMERIC_ID){
				...
				this.velY = snake.getVelY();
				setLastPosition(new Point2D(this.x,this.y));
				tail = new SnakeTail2(this,game, game.getBottomLayer(), new Circle(Settings.SECTION_SIZE-5,new ImagePattern(GameImageBank.snakeTail)), this.x, this.y, GameObjectID.SnakeTail, PlayerMovement.MOVE_LEFT);
				game.getObjectManager().addObject(tail);
				snake.setTail(tail);
				snake.setNeighbor(this);
				if(this.numericID == Player2.NUMERIC_ID){
				...
				this.velY = snake.getVelY();
				setLastPosition(new Point2D(this.x,this.y));
				tail = new SnakeTail2(this,game, game.getBottomLayer(), new Circle(Settings.SECTION_SIZE-5,new ImagePattern(GameImageBank.snakeTail)), this.x, this.y, GameObjectID.SnakeTail, PlayerMovement.MOVE_RIGHT);
				game.getObjectManager().addObject(tail);
				snake.setTail(tail);
				snake.setNeighbor(this);
				if(this.numericID == Player2.NUMERIC_ID){
				...
				this.x = (double) (x - this.circle.getRadius()*2);
				this.y = y;
				this.velX = snake.getVelX();
				this.velY = snake.getVelY();
				setLastPosition(new Point2D(this.x,this.y));
	...
	public void move() {
		if(Player2.killTheSnake == false && !Player2.levelComplete && Player2.keepMoving)
		super.move();
	...
	public void blowUp(){
		...
		for (int i = 0; i < Settings.PARTICLE_LIMIT; i++) {
			...
			game.getDebrisManager().addParticle(new SectionDisintegration(game, GameImageBank.snakeDebris2, particleLife, particleSize,(double) (x + this.radius/2), (double) (y + this.radius/2)));
		}
		blowUp = false;
		}
	}
