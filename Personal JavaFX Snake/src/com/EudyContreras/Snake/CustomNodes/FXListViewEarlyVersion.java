...
public class FXListViewEarlyVersion<T> {
	...
	private FXCallback<FXListViewEarlyVersion<T>, FXListCell<T>> callBack;
	...
	public FXListViewEarlyVersion(ObservableList<T> data, AddOrder order) {
	...
	public FXListViewEarlyVersion(ObservableList<T> data) {
		this(data,AddOrder.BOTTOM);
	}

	public FXListViewEarlyVersion(AddOrder order) {
		this(FXCollections.observableArrayList(),order);
	}

	public FXListViewEarlyVersion() {
	...
	private void observeChanges() {
		...
		container.vvalueProperty().addListener((obs, oldValue, newValue) -> {
			...
			if (value == scrollBar.getMax()) {
				...
				addCell((int) (listLayout.getChildren().size()*0.02));
...
//		endEvent.fireEvent();
	...
	public void setCellFactory(FXCallback<FXListViewEarlyVersion<T>, FXListCell<T>> callBack) {
	...
	public FXCallback<FXListViewEarlyVersion<T>, FXListCell<T>> getCellFactory() {
		return callBack;
	}
