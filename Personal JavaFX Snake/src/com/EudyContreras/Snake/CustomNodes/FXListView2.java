...
public class FXListView2<ITEM> {
	...
	private boolean allowLazyLoading = false;
	...
	private FXCallback<FXListView2<ITEM>, FXListCell<ITEM>> callBack;
	...
	public FXListView2(AddOrder order, ObservableList<ITEM> data) {
		this.addOrder = order;
...
//					indicator.setIndeterminate(false);
	...
	private void lazyLoading(boolean state) {
		if (state) {
			cullCells(container);
		} else {
			...
			case BOTTOM:
				for (int i = nodes.size() - 1; i >= 0; i--) {
					nodeCollection.add(0, (FXListCell<ITEM>) nodes.get(i));
				}
				break;
			case TOP:
				for (int i = nodes.size() - 1; i >= 0; i--) {
					nodeCollection.add((FXListCell<ITEM>) nodes.get(i));
				}
				break;
			}
	...
	public void setCellFactory(FXCallback<FXListView2<ITEM>, FXListCell<ITEM>> callBack) {
	...
	public FXCallback<FXListView2<ITEM>, FXListCell<ITEM>> getCellFactory() {
		return callBack;
	}
