...
/**
 * This class is used to handle clients and all actions from each client this
 * class organizes the logic used to send objects to specific clients and it
 * also handles the information that travels through the server.
 * 
...
public class ClientManager extends MultiplayerServer {
	...
	private HashMap<String, String> online_user_info; // Key: passWord, Value:
														// userName
	private HashMap<String, String> offline_user_info; // Key: passWord, Value:
														// userName
	 ...
	 * 
	 * @param GUI
	 *            the servers graphical user interface.
	 */
	...
	/**
	 * Receives a new connecting client and sends it to be revised for
	 * uniqueness.
	 * 
	 * @param client
	 */
	...
	/**
	 * This method is responsible for processing the login details of a new
	 * connecting client.
	 * 
	 * @param pack
	 * @param client
	...
	/**
	 * Sends a list containing all users that are currently online to all the
	 * online clients.
	 * 
	 * @param obj
	 * @throws ClassNotFoundException
	...
	/**
	 * Adds unique client to a list containing all clients that have logged into
	 * the server.
	 * 
	 * @param client
	 */
	...
	/**
	 * returns a list containing the username of all the users that have logged
	 * into the server.
	 * 
	 * @return
	 */
	...
	/**
	 * returns a list containing the username of all users that have
	...
	/**
	 * Method which sends a private chat confirmation this method will send a
	 * confirmation to the receiver and the starter of the private chat
	 * 
	 * @param pack
	 */
	...
	/**
	 * This method is used for sending updated list containing online users and
	 * offline users.
	 * 
	 * @param client
	 */
	...
	/**
	 * Removes a clients information from an online list and moves it into a
	 * offline client information list.
	 * 
	 * @param client
	 */
	...
	public boolean clientExist(String userName) {
		if (online_clients.containsKey(userName)) {
			return true;
		} else if (offline_clients.containsKey(userName)) {
			return true;
		} else {
			return false;
		}
	...
	public void disconnectClient(MultiplayerClient client) {
		...
		try {
			...
			sendBroadcastPackage(
					new ServerResponse(client.getUsername(), "All users", getAllUsers(), client.getUsername(), 1.0));
		} catch (ClassNotFoundException | IOException e) {
			e.printStackTrace();
		}
		client = null;
