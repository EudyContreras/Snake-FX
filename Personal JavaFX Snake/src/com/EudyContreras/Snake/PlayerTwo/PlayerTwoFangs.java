...
public class PlayerTwoFangs extends AbstractObject {
	...
	public void move() {
		...
		circle.setRadius(GameSettings.ALLOW_AI_CONTROLL ? GameSettings.PLAYER_TWO_SIZE * 0.40 : GameSettings.PLAYER_TWO_SIZE * 0.40);
	}
	...
	public void checkOffset() {
		if (snake.getCurrentDirection()== PlayerMovement.MOVE_UP) {
			this.offsetY = GameSettings.ALLOW_AI_CONTROLL ? -this.snakeHead.getRadius()*.65 : -this.snakeHead.getRadius()*.65;
			this.offsetX = 0;
		} else if (snake.getCurrentDirection() == PlayerMovement.MOVE_DOWN) {
			this.offsetY = GameSettings.ALLOW_AI_CONTROLL ? this.snakeHead.getRadius()*.65 : this.snakeHead.getRadius()*.65;
			this.offsetX = 0;
		} else if (snake.getCurrentDirection() == PlayerMovement.MOVE_LEFT) {
			this.offsetX = GameSettings.ALLOW_AI_CONTROLL ? -this.snakeHead.getRadius()*.65 : -this.snakeHead.getRadius()*.65;
			this.offsetY = 0;
